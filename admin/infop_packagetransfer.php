...
switch($action) 
{
	case "update":
			...
			$query=mysql_query("insert into package values('','".$startdate."','".$enddate."','".$country."','".$destinations."','".$num."','".$day."','".$night."','".$mprice."','".$points."','".$rename."','".$_SESSION[userid]."','".$username."','".$mobile."','".$wechat."','".$currdate."','".$link."')") or die(mysql_error());
			if($query){
				   echo "<script>alert('套餐转让发布成功！');location.href='infop_packagemain.php?action=packagetransfer';</script>";
				}else{
				   echo "<script>alert('套餐转让发布失败！');location.href='infop_packagemain.php';</script>";
				}
	        break;
	}//end case
?>
...
if (document.form1.mprice.value=="") {
alert ("请填写套餐会员价格 ！")
return false
}
...
<form  name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>?action=update" onsubmit="return checkdata()">
  <table id="form" width="579" height="188" border="1" align="center">
    <tr>
      <th colspan="2" scope="col">发布套餐转让信息</th>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><span class="STYLE1">
        <input name="mprice" type="text" id="mprice"/>
        美元
        *</span></td>
    </tr>
    <tr>
      <td align="center">积分抵扣：</td>
      <td><span class="STYLE1">
        <input name="points" type="text" id="points"/>
      美元*</span></td>
    </tr>
    <tr>
      <td align="center">更名费用：</td>
      <td><p>
        <label>
          <input type="radio" name="rename" value="0" />
          我付</label>
        <input name="rename" type="radio" value="1" checked="checked" />
        你付
        <br />
      </p></td>
    </tr>
    <tr>
      <td align="center">套餐链接：</td>
      <td><span class="STYLE1">
        <input name="link" type="text" id="link" size="60"/>
      </span></td>
    </tr>
    <tr>
      <td colspan="2" align="center" valign="middle" class="subtime"><p>
        <input type="submit" name="submit" value="发布" />
        <input name="userid" type="hidden" value="<?php echo $_SESSION['userid'] ?>" />
        <input name="username" type="hidden" value="<?php echo $username ?>" />
        <input name="mobile" type="hidden" value="<?php echo $mobile ?>" />
        <input name="wechat" type="hidden" value="<?php echo $wechat ?>" />
        <span class="STYLE1">注：标*为必添项</span></p>
      </td>
    </tr>
  </table>
</form>
