...
{
			case "delete":
					$sql = "delete from dmop_freemachine where machine_name = '".$machine."'";
			...
			case "batchdel":
					if(empty($_POST[checkbox])){
						...
						$sql = "delete from dmop_freemachine where id in ($delete_id) or pid in ($delete_id)"; 
						$query = mysql_query($sql) or die(mysql_error());
						if($query){
...
<?php
	$pg = & new turn_page();
	$sql = "select * from  dmop_freemachine  where pid ='0' order by idc,status,borrow_time desc" ;
    $query=mysql_query($sql) or die(mysql_error());
...
?>
    <tr>
      ...
      <td align="center"><a href="dmop_freemachineview.php?id=<?php echo $result["id"]?>"><?php echo $result["machine_name"]?></a></td>
      ...
      <td align="center"><font color='red'><?php if($result["status"] == '已借用'){ echo ceil((strtotime($result["return_time"])-strtotime($result["borrow_time"]))/3600/24);}else{ echo 0;}?></font></td>
      <td align="center"><a href="dmop_freemachineedit.php?id=<?php echo  $result["id"]?>">编缉</a></td>
      <td align="center" onClick="javascript:return window.confirm('确定要删除此信息吗？')"><a href="<?php echo $_SERVER['PHP_SELF']."?action=delete&machine=".$result[machine_name]?>">删除</a></td>
    </tr>
<?php
}
?>
