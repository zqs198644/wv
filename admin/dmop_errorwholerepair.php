...
#form2 .fristLine input{width:15px;border:none;}
</style>
</head>
<body>
<?php
...
switch($action) 
{
			case "repair":
					...
					mysql_query("update dmop_stat set status_whole = '2'") or die(mysql_error());
					if($query){
			...
			case "oper":
					...
					$query = mysql_query($sql) or die(mysql_error());
					mysql_query("update dmop_stat set status_whole = '3'") or die(mysql_error());
					if($query){
			...
			case "tosa":
					...
					$query = mysql_query($sql) or die(mysql_error());
					mysql_query("update dmop_stat set status_whole = '4'") or die(mysql_error());
					if($query){
			...
			case "finish":
					...
					$query = mysql_query($sql) or die(mysql_error());
					mysql_query("update dmop_stat set status_whole = '1'") or die(mysql_error());
					if($query){
...
?>
  <table align="center" width="880">
    ...
    <?php
	$pg = & new turn_page();
    $sql = "select *,machine_name,count(*) as num from dmop_error_disk where status='".$status."' group by machine_name having count(machine_name) > 3 order by idc,report_date desc";
	$query = mysql_query($sql) or die(mysql_error());
	$error_num = mysql_num_rows($query);
	$pg->file = $GLOBALS['PHP_SELF'];
    $pg->pvar = 'page';
	$page_num = '10';
    $pg->set($page_num,$error_num,0);
    $limit = $pg->limit();
    $sql = $sql ." limit ".$limit;
	$query=mysql_query($sql) or die(mysql_error());
	if( $error_num == '0' ){
?>
    <tr>
      <td colspan="11"  align="center" class="firstLine">目前没有要报修的机器
...
?>
    <tr>
      <td class="bottomtd" align="center" valign="middle"><input type="submit" name="Submit3"  onClick="javascript:return window.confirm('确定要<?php echo $onclick?>批机器吗？')" value="<?php echo $onclick?>" /></td>
      <td class="bottomtd1" colspan="10" align="right"> <input type="submit" onClick="javascript:loca('dmop_export.php?sql=<?php  
	  ...
	  echo urlencode($sql);
	 ?>')" name="export" value="导出机器列表" /><? echo $page_num.'台/页 共<font color=red>'.$error_num.'</font>台 '.$pg->output(1);?></td>
    </tr>
  </table>
</form>
