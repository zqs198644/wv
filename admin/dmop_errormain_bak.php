...
<form id="form2" name="form2" method="post" action="<?php echo $_SERVER['PHP_SELF']?>?action=deluser">
  <table width="890" align="center">
    ...
    <?php
	$pg = & new turn_page();
    $sql = "select * from  dmop_error_disk where machine_name in (select machine_name from (SELECT machine_name,count(*) as a FROM dmop_error_disk WHERE status='报修'  group by machine_name order by report_date) as b where a <= 3 ) and status='报修'  order by report_date desc" ;
    $query=mysql_query($sql) or die(mysql_error());
	$num = mysql_num_rows($query);
    ...
    $limit = $pg->limit();
//	$sql = "select * from  dmop_error_disk where machine_name in (select machine_name from (SELECT machine_name,count(*) as a FROM dmop_error_disk WHERE status='报修'  group by machine_name order by report_date) as b where a <= 3 ) and status='报修'  order by idc,report_date desc limit ".$limit."" ;
    $sql = $sql ." limit ".$limit;
	$query = mysql_query($sql) or die(mysql_error());
...
?>
    <tr>
      <td  align="center" class="firstLine"><label>
